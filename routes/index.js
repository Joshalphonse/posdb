...
router.post('/update',function(req,res,next){
  var username = req.session.username;
  var priv = req.session.privledge;
  var data = req.body; //id, name,price,category_id,calories
  ////console.log(data);
  if(username && (priv == "Admin" || priv == "Manager")){
    db.serialize(function(){
      if(data.id == ""){
        var stmt = db.prepare("INSERT INTO PRODUCT (calories, "+
        "CATEGORY_ID,PRODUCT_NAME,PRICE) VALUES ($calories,$cat,$name,$price)");
        var param = {$calories:data.calories,$cat:data.category_id,$name:data.name,
                     $price:data.price*100};
      }else{
        var stmt = db.prepare("UPDATE PRODUCT SET calories=$calories, "+
  ...
  if(username && (priv == "Admin" || priv == "Manager")){
    db.serialize(function(){
      if(data.id == ""){
        ...
        var stmt = db.prepare("UPDATE CATEGORY SET PARENT_CATEGORY_ID=$calories, "+
